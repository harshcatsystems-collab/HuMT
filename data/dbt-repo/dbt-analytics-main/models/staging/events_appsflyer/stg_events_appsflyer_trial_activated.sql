...
source as (
    select
        *
    from {{ source('events_appsflyer', 'trial_activated') }}
),
...
renamed_and_rank as (
    select
        ...
        sent_at as event_at,
        PARSE_JSON(event_value):content_ids::STRING AS acq_trial_content_id,
        original_timestamp,
        row_number() over (
            partition by
                date(event_at),
...
-- Select only the latest event for each user per day
select
    *
from
    renamed_and_rank
where
    rn = 1
    -- useful for deubgging
    -- and user_id = '6160cdc90462c35c08a63904'