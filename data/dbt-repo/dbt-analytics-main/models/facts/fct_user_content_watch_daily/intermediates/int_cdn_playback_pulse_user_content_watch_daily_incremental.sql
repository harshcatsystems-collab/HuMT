{{
    config(
        ...
        unique_key=['user_id', 'content_id', 'watch_date_utc'],
        incremental_strategy='merge',
        on_schema_change='fail'
    )
}}

with cdn_playback_pulse as (
    select 
        *
    from  
    ...
    where 
        ...
        {% if is_incremental() %}
        -- Only process last 2 days of data on incremental runs
        and date(event_timestamp_utc) >= (
